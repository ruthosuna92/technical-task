...
>
    ...
    <div class="flex justify-between gap-6 xs:gap-2">
                        ...
                        {#if therapist.profile.yearsOfExperience !== null}
                            <div class=" flex items-center gap-2 xs:gap-1.5">
                                ...
                                <p class="text-right text-base xs:text-sm">
                                    ...
                                    {therapist.profile.yearsOfExperience > 0
                    ...
                    >
                        <div class="flex items-center gap-2 xs:gap-1.5">
                            {#if Object.keys(therapist.demographic).length > 0}
                                ...
                                <p class="xs:text-sm">
                                    {therapist.demographic.location}, GMT -7
                                </p>
                            {/if}
                        </div>
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-base w-full xs:text-sm">
                                    ...
                                    {#if Array.isArray(therapist.demographic.mainLanguage)}
                                        ...
                                        {/each}
                                    {:else}
                                        {therapist.demographic.mainLanguage}
                                    {/if}
                                </span>
                            </div>
                        {/if}
        ...
        <!-- Like button -->
        <button class="flex justify-end w-8 xs:w-10 h-8 xs:h-10 ">
        <svg
            class="flex justify-end w-8 xs:w-10 h-8 xs:h-10 xs:stroke-medium-gray stroke-primary-dark"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 30 30"
            fill="none"
        >
    ...
    <div class="flex gap-2 flex-wrap">
        ...
        <a
            class="flex py-0 {therapist.profile.hasOwnProperty('topAreas')
                ? 'px-4'
                : 'px-0'} xs:px-0 items-center gap-[0.3125rem] text-accent-dark-blue xs:text-smb"
            href={'/therapistBio/'+ therapist.id }
        >
            View profile
        </a>
    </div>
...
<!--  <p>Click <a href={link({ pathname: '/dynamic/123' })}>here</a> to go to a dynamic route with ID 123.</p> -->